...
const ChatAI = () => {
  const geminiApiKey = "AIzaSyDFtWCkXuRmnxAHf_TcWj5DD3jbBsyzVrU";
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!geminiApiKey) {
        throw new Error(
          "API ключ відсутній. Будь ласка, встановіть змінну середовища REACT_APP_GEMINI_API_KEY."
        );
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Помилка ${response.status}: ${
            errorData.error?.message || "Невідома помилка"
          }`
        );
      }
